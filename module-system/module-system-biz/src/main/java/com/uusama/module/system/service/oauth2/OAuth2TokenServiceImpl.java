...
public class OAuth2TokenServiceImpl implements OAuth2TokenService {
    ...
    private OAuth2RefreshTokenDO createOAuth2RefreshToken(Long userId, Integer userType, OAuth2ClientDO clientDO, List<String> scopes) {
        OAuth2RefreshTokenDO refreshToken = OAuth2RefreshTokenDO.builder().refreshToken(generateRefreshToken())
                .userId(userId).userType(UserTypeEnum.of(userType))
                .clientId(clientDO.getClientId()).scopes(scopes)
                .expiresTime(LocalDateTime.now().plusSeconds(clientDO.getRefreshTokenValiditySeconds())).build();
