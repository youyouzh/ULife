...
public class DeptServiceImpl implements DeptService {
    ...
    public void deleteDept(Long id) {
        ...
        // 校验是否有子部门
        ParamUtils.checkNotMatch(deptMapper.selectCountByParentId(id) > 0, DEPT_EXITS_CHILDREN);
        // 删除部门
    ...
    private void validateParentDeptEnable(Long id, Long parentId) {
        ...
        // 不能设置自己为父部门
        ParamUtils.checkNotMatch(parentId.equals(id), DEPT_PARENT_ERROR);

        // 父岗位不存在
        DeptDO dept = deptMapper.selectById(parentId);
        ParamUtils.checkNotNull(dept, DEPT_PARENT_NOT_EXITS);

        // 父部门被禁用
        ParamUtils.checkMatch(dept.getState().isEnable(), DEPT_NOT_ENABLE);

        // 父部门不能是原来的子部门
        List<DeptDO> children = getDeptListByParentIdFromCache(id, true);
        ParamUtils.checkNotMatch(children.stream().anyMatch(dept1 -> dept1.getId().equals(parentId)), DEPT_PARENT_IS_CHILD);
    }
    ...
    private void validateDeptExists(Long id) {
        ...
        ParamUtils.checkNotNull(dept, DEPT_NOT_FOUND);
    }
    ...
    private void validateDeptNameUnique(Long id, Long parentId, String name) {
        ...
        ParamUtils.checkNotNull(id, DEPT_NAME_DUPLICATE);
        ParamUtils.checkMatch(menu.getId().equals(id), DEPT_NAME_DUPLICATE);
    }
