...
import com.uusama.common.util.NumberUtil;
...
public class CharSequenceUtil {
    ...
    public static int indexOf(CharSequence text, CharSequence searchStr, int from, boolean ignoreCase) {
        ...
        for (int i = from; i < text.length(); i++) {
            if (CharSequenceUtil.isSubEquals(text, i, searchStr, 0, searchStr.length(), ignoreCase)) {
                return i;
            }
        }
        return INDEX_NOT_FOUND;
     ...
     *
     * @param str    字符串
     * @param filter 过滤器，{@link Predicate#test(Object)}返回为{@code true}的保留字符
     * @return 过滤后的字符串
     * @since 5.4.0
     */
